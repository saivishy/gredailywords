...
slackToken='<INSERT API TOKEN HERE>'
slackobj=Slacker(slackToken)
channel_name='<INSERT CHANNEL NAME HERE>'
...
def post_word(): 
    ...
    xlsheet=pd.read_excel('Words with mnemonics (1-48).xlsx',sheet_name='day 1',index_col=0,dtype={'WORDS': str, 'DEFINITION': str,'MNEMONIC':str,'SYNONYMS':str,'PHONETIC':str})
    ...
    xlsheet=xlsheet[1:]
    current_word=get_var('cur_word.p')
    msg=str(current_word)+'.\n\n *_'+xlsheet[col[0]][current_word].upper()+'_*\n\n  '+xlsheet[col[1]][current_word]+'\n\n *SYNONYMS* : '+xlsheet[col[3]][current_word]+'\n\n *MNEMONIC* : '+xlsheet[col[2]][current_word]
    slackobj.chat.post_message('#' + channel_name, msg, as_user=False)
    word_list=get_var('word_list.p')
    word_list.append(xlsheet[col[0]][current_word])
    current_index=get_var('index.p')
    current_index+=1
    current_word+=1
    dump_var(current_index,'index.p')
    dump_var(current_word,'cur_word.p')
    dump_var(word_list,'word_list')
